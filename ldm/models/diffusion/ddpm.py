...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, control, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            # cond[int(t*self.prospect_stages/1000):int(t*self.prospect_stages/1000)+3]
            x_recon = self.model(x_noisy, t, control, **cond)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond[int(t*self.prospect_stages/1000):int(t*self.prospect_stages/1000)+3], control=None)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t,control, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            # print("c_crossattn:",len(c_crossattn))
            # print(c_crossattn[0].size())
            # dsa
            # cc = torch.cat(c_crossattn, 1)
            # print(len(c_crossattn))
            # print(c_crossattn[0].size())
            # dsa
            out = self.diffusion_model(x, t,  control=control, context=c_crossattn)
